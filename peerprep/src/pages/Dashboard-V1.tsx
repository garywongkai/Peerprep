...
function Dashboard() {
  ...
  const [matchmakingService, setMatchmakingService] =
    useState<MatchmakingService | null>(null);
  ...
  const checkDeclinedMatch = async () => {
    if (matchmakingService) {
      const wasDeclined =
        await matchmakingService.checkAndHandleDeclinedMatch();
      if (wasDeclined) {
        setMatchDeclined(true);
        setOpenMatchDialog(false);
        setIsMatched(false);
        setAlertContent(
          "Your last match was declined. You can start a new match when you're ready."
        );
        setAlert(true);
      }
    }
  ...
  useEffect(() => {
    if (matchDeclined) {
      setAlertContent(
        "Your last match was declined. You've been returned to the waiting queue."
      );
      setAlert(true);
  ...
  const handleConfirmMatch = async (confirm: boolean) => {
    if (matchData && matchmakingService) {
      if (confirm) {
        ...
        handleCancel();
        setAlertContent(
          "You've declined the match. You can start a new match when you're ready."
        );
        setAlert(true);
      }
    }
  ...
  useEffect(() => {
    ...
    const matchQuery1 = query(
      ...
      where("status", "in", ["active", "pending", "canceled"])
    );
    ...
    const unsubscribe1 = onSnapshot(matchQuery1, (snapshot) => {
      if (!snapshot.empty) {
        ...
        match.userId2 === user.uid
          ? setMatchedUserName(match.userName1)
          : setMatchedUserName(match.userName2);
        setMatchData(match);
        setOpenDialog(false);
        setOpenMatchDialog(true);
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        <DialogTitle>
          You are matching for <strong>{difficulty}</strong> questions
        </DialogTitle>
        <DialogContent>
          <p>
            Click <strong>Cancel</strong> to cancel matchmaking
          </p>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={openMatchDialog} onClose={() => handleConfirmMatch(false)}>
        ...
        <DialogContent>
          ...
          <p>
            You have been matched for a {matchData?.difficulty} level problem.
          </p>
