...
const updateUserProfile = (req, res) => {
    ...
    if (!displayName && !photoURL) {
        return res
            .status(422)
            .json({ error: "Display name and profile image cannot be empty." });
    }
    if (!displayName) {
        return res.status(422).json({ error: "Display name cannot be empty." });
    }
    if (!photoURL) {
        return res
            .status(422)
            .json({ error: "Profile image cannot be empty." });
    }
    ...
    updateProfile(user, {
        ...
        .then(() => {
            res.status(200).json({
                message: "Profile updated successfully. All changes saved.",
            });
        })
        .catch((error) => {
...
const getUserAttemptHistory = async (req, res) => {
    ...
    try {
        ...
        const userRef = dbAdmin.collection("users").doc(uid);
        const attemptsSnapshot = await userRef
